...
//if($conexao) mysql_select_db("sico", $conexao) or die("O banco solicitado não pode ser utilizado :  . mysql_error()"); else echo "não foi possivel estabelecer uma conecção";

//se for passada também a variável atualiza, ele entende que foi realizado o upload para o uc_sem_med2 com a lista da TELLUS e executa a rotina abaixo
if($_GET["atualiza"]){
$query = "update uc_sem_med set status = '1' and num_uc not in (select num_uc from uc_sem_med2)";
$result = mysql_query($query) or die(mysql_error());
$query = "update uc_sem_med inner join uc_sem_med2 on uc_sem_med.num_cliente=uc_sem_med2.num_cliente set uc_sem_med.local = uc_sem_med2.tensao";
$result = mysql_query($query) or die(mysql_error());
header('Location: uc_s_medicao.php?success=1'); 
die;
}

//caso seja só o download ele executa essa parte do codigo
$query = "select seq, num_cliente, num_uc, nome, cpf_cnpj, endereco, regiao, classe, tensao, tipo_ligacao, data_ligacao, local, medidor, grupo, kwh_estimado, situacao from uc_sem_med2 where not exists (select seq, num_cliente, num_uc, nome, cpf_cnpj, endereco, regiao, classe, tensao, tipo_ligacao, data_ligacao, local, medidor, grupo, kwh_estimado, situacao from uc_sem_med Where uc_sem_med2.num_cliente=uc_sem_med.num_cliente) UNION select seq, num_cliente, num_uc, nome, cpf_cnpj, endereco, regiao, classe, tensao, tipo_ligacao, data_ligacao, local, medidor, grupo, kwh_estimado, situacao from uc_sem_med where status = '0' order by seq ASC";
$result = mysql_query($query) or die(mysql_error());
...
$csv_filename = 'db_uc_sem_med_'.date('Y-m-d').'.csv';
...
//move o arquivo do tmp para a pasta que queremos trabalhar
    if(is_uploaded_file($_FILES['csv']['tmp_name'])) move_uploaded_file($_FILES['csv']['tmp_name'], "tmp_arquivo_upload_uc_s_med.csv");
  
 // após o procedimento ele redireciona o rapaz para outra pag.

$query = "TRUNCATE TABLE uc_sem_med2";
$result = mysql_query($query) or die(mysql_error());
$query = "LOAD DATA LOCAL INFILE 'tmp_arquivo_upload_uc_s_med.csv' INTO TABLE uc_sem_med2 FIELDS TERMINATED BY ';' ESCAPED BY '\\\\' LINES TERMINATED BY '\\n' IGNORE 1 LINES (`seq`,`num_cliente`,`num_uc`,`nome`,`cpf_cnpj`,`endereco`,`regiao`,`classe`,`tensao`,`tipo_ligacao`,`data_ligacao`,`local`,`medidor`,`grupo`,`kwh_estimado`,`situacao`,`id`,`status`)";
$result = mysql_query($query) or die(mysql_error());

//deletar na tabela registro do mes caso exista
$hoje = date("m"); 
$query = "DELETE FROM uc_sem_med_oficial where month(data_insercao) = '$hoje'";
$result = mysql_query($query) or die(mysql_error());
//carrega na tabela oficial o novo mês
$query = "LOAD DATA LOCAL INFILE 'tmp_arquivo_upload_uc_s_med.csv' INTO TABLE uc_sem_med_oficial FIELDS TERMINATED BY ';' ESCAPED BY '\\\\' LINES TERMINATED BY '\\n' IGNORE 1 LINES (`seq`,`num_cliente`,`num_uc`,`nome`,`cpf_cnpj`,`endereco`,`regiao`,`classe`,`tensao`,`tipo_ligacao`,`data_ligacao`,`local`,`medidor`,`grupo`,`kwh_estimado`,`situacao`,`id`,`status`)";
$result = mysql_query($query) or die(mysql_error());


$query = "update uc_sem_med_oficial inner join uc_sem_med on uc_sem_med_oficial.num_cliente=uc_sem_med.num_cliente set uc_sem_med_oficial.status = uc_sem_med.status where month(uc_sem_med_oficial.data_insercao) = '$hoje'";
$result = mysql_query($query) or die(mysql_error());

$query = "DELETE FROM uc_sem_med_oficial WHERE tipo_ligacao = '0'";
  $result = mysql_query($query) or die(mysql_error());

/*
//Função utilizada apenas para alterar data de insercao uc sem med

  $query = "update uc_sem_med_oficial set data_insercao = ('2018/10/17 18:10:01') 
WHERE month(data_insercao) = '01'";
$result = mysql_query($query) or die(mysql_error());
*/


header('Location: uc_s_medicao.php?success=1'); die;
}
...
<body>
  ...
  <a class="navbar-brand" id="title01" href="http://10.68.14.67/novosicob/">
    <div class="h4"> UC Sem Medição </div>
  </a>
</nav>
      <div class="container-fluid">
...
<?
          if ((!$_GET['upload']) && empty($_GET[success])){ ?> 

          <figure id="figure_relat01"><a href="http://10.68.14.67/novosicob/uc_s_medicao.php?upload=1"><img src="imagens/bd02.png" alt="imagem" class="img-thumbnail img_link_gerar_relat"><figcaption><div class="sombra_caption2">Enviar arquivo atualizado pelo GCO a ser filtrado [passo 1]</div></figcaption></a></figure>

          <figure id="figure_relat01"><a href="http://10.68.14.67/novosicob/uc_s_medicao.php?download=1"><img src="imagens/relat13.jpg" alt="imagem" class="img-thumbnail img_link_gerar_relat"><figcaption><div class="sombra_caption2">Baixar arquivo em .CSV com os filtros aplicados (arquivo pronto) [passo 2]</div></figcaption></a></figure>   

<? } else if (($_GET['upload'])) { ?>
  <div style="margin-top: 100px; margin-left: 250px;">
          <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1"> Selecione o arquivo CSV puro: <input name="csv" type="file" id="csv"/><br><br>
...
<? } ?>
      ...
      <?php
          if (!empty($_GET[success])) { ?>
            <div style="margin-top: 100px; margin-left: 370px;">
            <b>  Seu arquivo foi carregado com sucesso! </b>    
            <br/>
            <br/>
            <a href="http://10.68.14.67/novosicob/uc_s_medicao.php?upload=1">Enviar novo arquivo de UC Sem Medição</a>
            </div>        
      <? } ?>
