...
$urlref = 'http://localhost/novosicob/eusd_load.php?upload=1';
$urlref2 = 'http://10.68.14.67/novosicob/eusd_load.php?upload=1';
...
$sql = "SELECT eusd FROM eusd LIMIT 1";
$result = mysql_query($sql);
$registro = mysql_fetch_array($result);
$eusd = $registro['eusd'];
...
/*if(!is_null($estproces)){
  echo "<script language='javascript' type='text/javascript'>alert('Voc\u00ea ainda n\u00e3o finalizou o passo 2 referente ao ultimo processo. Para prosseguir por favor complete primeiro o envio do Anexo III');window.location.href='anexo_load2.php?upload=1'</script>";
  break;
...
$filename1 = "relatorio_anexo_iii\\eusd_original\\temp.csv";


if (file_exists($filename1)){
    echo "<script language='javascript' type='text/javascript'>alert('Apague o arquivo cujo nome \u00e9 temp do diretorio eusd_original e tente fazer o upload novamente. Caso o problema persista, por favor, contate o administrador do sistema. ');window.location.href='eusd_load.php?upload=1'</script>";
  exit();} 
...
//Buscando data referente ao ultimo arquivo anexo III armazenado no banco de dados
  ...
  $anoref = $registro['ano'];
  $diaref = 01;
  $dataref = "$anoref-$mesref-$diaref";


$arquivo = fopen ("$filename1", 'r');
...
if ($ii > 1 && $ii < 3) {
  ...
  $mes_Eusd = $converte_array[2];

/*
  $anoeEusdConvert = substr($mes_Eusd, 4, 2);

  switch ($anoeEusdConvert) {
    case '15':
      $anoEusd = "2015";
      break;
      case '16':
...
*/
 ...
 $anoEusd = $_POST['anoEusd'];
 $anoEusd2 = substr($anoEusd, 2, 2);
//$anoEusd2 = $anoeEusdConvert;


if (stristr($mes_Eusd, "JAN")){
  $mes_Eusd = 'JAN_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "FEV")){
  $mes_Eusd = 'FEV_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "MAR")){
  $mes_Eusd = 'MAR_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "ABR")){
  $mes_Eusd = 'ABR_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "MAI")){
  $mes_Eusd = "MAI_"."$anoEusd2";
} 
if (stristr($mes_Eusd, "JUN")){
  $mes_Eusd = 'JUN_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "JUL")){
  $mes_Eusd = 'JUL_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "AGO")){
  $mes_Eusd = 'AGO_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "SET")){
  $mes_Eusd = 'SET_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "OUT")){
  $mes_Eusd = 'OUT_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "NOV")){
  $mes_Eusd = 'NOV_'."$anoEusd2";
} 
if (stristr($mes_Eusd, "DEZ")){
  $mes_Eusd = 'DEZ_'."$anoEusd2";
} 
...
fclose($arquivo);
 ...
 $sql = "SELECT cliente FROM eusd WHERE data_ref = '$mes_eusd'";
 $result = mysql_query($sql);
 $registro = mysql_fetch_array($result);
...
//if (!is_null($confreg)){
//  unlink("eusd_original\\temp.csv");
//echo "<script language='javascript' type='text/javascript'>alert('J\u00e1 existe um arquivo referente a $mes_Eusd em nosso banco de dados. Por favor, confira o arquivo que voc\u00ea est\u00e1 enviando ou contate o administrador do sistema');window.location.href='eusd_load.php?upload=1'</script>";
//  exit();
//}
...
while($arqi <= 6){  
  ...
  exit();} 
  $filename2 = "relatorio_anexo_iii\\eusd_original\\Eusd original - $usuario $mes_Eusd edicao $arqi.csv";  
  $filename2_2 = "Eusd original - $usuario $mes_Eusd edicao $arqi.csv";
...
$arquivo = fopen ("$filename2", 'r');

$arqi = 1;
while($arqi <= 6){  
      if ($arqi == 6){
      echo "<script language='javascript' type='text/javascript'>alert('N\u00famero m\u00e1ximo de arquivos deste m\u00eas alcan\u00e7ado. Se voc\u00ea gostaria de fazer o upload de mais um arquivo, por favor v\u00e1 at\u00e9 o diret\u00f3rio e apague um dos arquivos referentes a este m\u00eas ou entre em contato com o servi\u00e7o de administra\u00e7\u00e3o do sistema');window.location.href='eusd_load.php?upload=1'</script>";
      exit();} 
      $filename3 = "relatorio_anexo_iii\\eusd_modificado\\Eusd Modificado - $usuario $mes_Eusd edicao $arqi.csv";
      $filename3_3 = "Eusd Modificado - $usuario $mes_Eusd edicao $arqi.csv";
...
$text = $converte_array[0].";".$converte_array[1].";"."Mes_eusd".";"."AnoEUSD".";\n";
...
if ($ii > 1) {
  ...
  $num_eusd = $converte_array[1]; 

  $num_eusd = trim($num_eusd);
  $retirar = array("R$", "U$", "$");
  $num_eusd = str_replace($retirar, "", "$num_eusd");

  $num_eusd = str_replace(".", "", "$num_eusd");


  $num_eusd = str_replace(",", ".", "$num_eusd");


  $text = "".$converte_array[0].";".$num_eusd.";".$mes_Eusd.";".$anoEusd.";\n";
  fwrite($arquivo2, $text);
...
$query = "LOAD DATA LOCAL INFILE 'relatorio_anexo_iii\\\\eusd_modificado\\\\Eusd Modificado - $usuario $mes_Eusd edicao $arqi.csv' INTO TABLE eusd FIELDS TERMINATED BY ';' ESCAPED BY '\\\\' LINES TERMINATED BY '\\n' IGNORE 1 LINES (`cliente`,`eusd`,`data_ref`,`anoEusd`)";
$result = mysql_query($query) or die(mysql_error());
$query = "DELETE FROM eusd WHERE cliente = '0'";
$result = mysql_query($query) or die(mysql_error());

//$sql = "UPDATE anexo_iii SET estproces = 1 WHERE estproces = 0";
//$result = mysql_query($sql) or die(mysql_error());

$sql = "INSERT INTO uploads(id, nomeArquivo, NomeUsuario, dataInsert) VALUES ('', '$nomeArquivo', '$nomeUsuario', '$dataInsert')";
$result = mysql_query($sql) or die(mysql_error());

header('Location: eusd_load.php?success=1'); die;
...
function optionCheck(){
 ...
 alert("Selecione a Eusd correspondente a data <?php echo "$mesref/$anoref" ?> ");
          }
  ...
  <a class="navbar-brand" id="title01" href="http://10.68.14.67/novosicob/">
       ...
       <div class="container-fluid">
        ...
        <div class="row">
           ...
           <div id="table04"> 
                                  ...
                                  <!-- criação de hiperlink para acessar página de consulta e exclusão de usuário -->
    ...
    <?php
      if ($_GET['upload'] && empty($_GET[success])){ ?> <form action="" onsubmit="startProgres();" style="display:block;" method="post" enctype="multipart/form-data" name="form1" id="form1" > 
                 ...
                 <div id="message" style="font-size:14px; font-family:'Open Sans';" >Selecione o arquivo EUSD:</div>
    ...
    <select name="anoEusd" size="1" class="formulario" id="ano" style=" clear:both; float:left; margin-left: 4px; margin-top:5px;" required>                    
      ...
      <input type="submit" name="Submit" id="Enviar"  value="Enviar"/>  
        ...
        <?php
             ...
             <a href="anexo_load2.php?upload=1">+ Enviar arquivo Anexo_III para o banco de dados [passo 2]</a><br/>
